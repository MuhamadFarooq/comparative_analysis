...
Viewport predictions were done for next frame, 1, 2, 3, 5 and 8 seconds. For next frame viewport prediction, MLP model gives higher accuracy (99%) while LSTM accuracy is 93.57%.
For long-term predictions, LSTM perfoms better than other ML models, also sequential models (LSTM, GRU, RNN) have higher accuracy than non-sequential models (LR, MLP).
